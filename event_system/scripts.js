...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (openBtn) {
    openBtn.addEventListener("click", () => {
      ...
      // clear previous fields so nothing auto fills
  ...
  if (starContainer) {
    starContainer.innerHTML =
      [...Array(5)].map((_, i) => `<span data-rate="${i+1}">★</span>`).join("");

    document.querySelectorAll("#rating-stars span").forEach(star => {
      star.addEventListener("click", () => {
        let rating = star.dataset.rate;
        document.getElementById("contact-rating").value = rating;

        document.querySelectorAll("#rating-stars span").forEach(s => {
          s.style.color = s.dataset.rate <= rating ? "gold" : "#ccc";
        });
      });
    });
  }
  ...
  if (sendBtn) {
    sendBtn.addEventListener("click", async () => {
      ...
      const rating = document.getElementById("contact-rating").value;
      const resBox = document.getElementById("contact-response");

      if (!message) {
        resBox.textContent = "Please enter your message.";
        resBox.style.color = "red";
        return;
      }

      const res = await apiCall("send_feedback", {
        name,
        email,
        message,
        rating
      ...
      if (res.success) {
        resBox.textContent = "Thank you! Your message was sent.";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (exportBtn) {
      exportBtn.addEventListener("click", () => {
          window.location.href = "php/api_handler.php?action=download_backup";
      });
  }
